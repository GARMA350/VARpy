...
Ttab = readtable('data_nueva_trimestral.xlsx', 'Sheet', 'Hoja1');
...
mnem = {'igae','sub','tasa','exp','tcr','mps'};
...
names_raw = {'IGAE (nivel, promedio trim.)','Inflaci\''on (sub)','Tasa de inter\''es', ...
             'Exp. Inflaci\''on','TCR (log, L\_TCR)','mps (instrumento, suma trim.)'};
...
sgtitle('Series originales trimestrales (antes de transformar)', 'Interpreter', 'latex');
...
actividad = 100*igae_cycle;        % brecha en % (log-desviación * 100)
...
brecha_tcr = 100*tcr_cycle;        % brecha en % (log-desviación * 100)
...
%% 1.2) Chequeo de estacionariedad (ADF + KPSS) de las brechas
% ------------------------------------------------------------------------
...
if exist('adftest','file') == 2 && exist('kpsstest','file') == 2
    ...
    fprintf('Brecha del producto -> ADF:  h = %d (1=estacionaria), p = %.4f | KPSS: h = %d (0=estacionaria), p = %.4f\n', ...
        h_adf, p_adf, h_kpss, p_kpss);
    fprintf('Brecha del TCR      -> ADF:  h = %d (1=estacionaria), p = %.4f | KPSS: h = %d (0=estacionaria), p = %.4f\n', ...
        h_adf_t, p_adf_t, h_kpss_t, p_kpss_t);

    if h_adf == 0 || h_kpss == 1
        warning(['La brecha del producto ("%s") NO pasa las pruebas de ' ...
                 'estacionariedad de forma concluyente. Revisa el lambda del ' ...
                 'filtro HP o considera una transformación adicional.'], act_label);
    end
    if h_adf_t == 0 || h_kpss_t == 1
        warning(['La brecha del TCR ("%s") NO pasa las pruebas de ' ...
                 'estacionariedad de forma concluyente. Revisa el lambda del ' ...
                 'filtro HP o considera una transformación adicional.'], tcr_label);
    end
else
    warning(['adftest/kpsstest no disponibles (requieren Econometrics ' ...
              'Toolbox); revisa la estacionariedad de las brechas manualmente ' ...
...
Xvnames = {'Brecha del Producto','Inflaci\''on (sub)','Tasa de inter\''es','Exp. Inflaci\''on','Brecha del TCR'};

nvars = size(X,2);

%% 2) Selección de rezagos (sobre la muestra completa)
% ------------------------------------------------------------------------
pmax = 8;               % 8 trimestres = 2 años a probar; ajusta si quieres más
...
n_ventanas = 3;
ventana_ini = [datetime(2008,1,1), datetime(2012,1,1), datetime(2016,1,1)];
ventana_fin = [datetime(2018,12,31), datetime(2022,12,31), datetime(2026,12,31)];
ventana_lbl = {'2008-2018', '2012-2022', '2016-2026'};
...
VARopt.mnem      = Xmnem;
VARopt.nsteps    = 20;         % horizonte de los IRF: 20 TRIMESTRES (~5 años)
VARopt.frequency = 'q';        % trimestral
VARopt.impact    = 0;
VARopt.pctg      = 90;
VARopt.method    = 'wild';
VARopt.inference = 1;
...
%% 9) Descomposición histórica (Historical Decomposition) — ÚLTIMA ventana
% ------------------------------------------------------------------------
% Solo se reporta para la ventana más reciente (2016-2026). Usar HDfp
% (Fry-Pagan), no HDmed, por la misma razón que en FEVD: es la que
% garantiza que las contribuciones de los choques sumen el total observado.
%
% Se fija VARopt.pick explícitamente para CADA variable antes de llamar a
% VARhdplot: la sección 7.1 deja VARopt.pick = nvars (la última variable,
% brecha del TCR) tras su loop de IRFs, y si no se reasigna aquí,
% VARhdplot solo grafica esa variable en vez de las 5.
%
% NOTA de interpretación: solo la contribución del choque 1 (política
% monetaria) tiene lectura estructural sin ambigüedad; las contribuciones
% de demanda/oferta/expectativas/tipo de cambio dependen de la matriz R
% de restricciones de signo (sección 5) y de la rotación aceptada en el
% bootstrap, así que interprétalas con esa salvedad.
%
% NOTA adicional (brecha del TCR): al trabajar con la brecha (componente
% cíclico HP) en lugar del nivel/log-nivel, la tendencia de largo plazo del
% tipo de cambio real ya fue removida ANTES de entrar al VAR. Por lo tanto,
% "Const." + "Init. cond." en esta descomposición ya NO deben leerse como
% la tendencia estocástica del TCR (a diferencia de si se usara el log-nivel
% directamente); aquí representan la parte no explicada por los choques
% dentro de la dinámica de la brecha misma.
VARopt.vnames  = Xvnames;
VARopt.figname = sprintf('graphics/svar_sign_iv_trimestral_%s', strrep(ventana_lbl{k_ultima}, '-', '_'));
for i = 1:nvars
    VARopt.pick = i;
