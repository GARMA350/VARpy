...
%
%  BLOQUE EXOGENO (EU): brecha de producto de EU (IPI, filtro HP), inflación
%  EU (log-diff de CPI) y FFR entran como controles de forma reducida junto
%  con la dummy COVID, vía el argumento EXOG de VARmodel. No participan en
%  la identificación estructural (ver principio en el handbook, Sec. IV.G).
%
...
ipi = Ttab.ipi;         % IPI de EU (nivel) -> se transforma a brecha en 1.1c
inf_eu = Ttab.cpi;      % CPI de EU (NIVEL, FRED CPIAUCSL) -> se transforma a inflación en 1.1d
ffr = Ttab.srf;         % Fed Funds Rate (nivel)
...
%% 1.1c) Brecha de producto de EU (IPI, filtro HP)
% ------------------------------------------------------------------------
% Mismo lambda_hp trimestral (14400) usado para igae y tcr, por consistencia
% metodológica dentro del mismo sistema.
log_ipi = log(ipi);
[ipi_trend, ipi_cycle] = hpfilter(log_ipi, 'Smoothing', lambda_hp);
% Sintaxis antigua: [ipi_trend, ipi_cycle] = hpfilter(log_ipi, lambda_hp);

brecha_ipi_eu = ipi_cycle;
ipi_label = 'Brecha del producto EU (IPI, filtro HP, \lambda=14400)';

%% 1.1d) Inflación EU (a partir del nivel de CPI)
% ------------------------------------------------------------------------
% inf_eu = Ttab.cpi (FRED CPIAUCSL) es un NIVEL -> casi seguro no
% estacionario en niveles. Se transforma a variación log trimestral
% (inflación EU), consistente con el tratamiento de 'inf' doméstica.
% NOTA: esto introduce un NaN en la primera observación de toda la
% muestra (ver ajuste de idx{k} en la sección 3, que descarta esa fila
% de cualquier ventana que la incluya, siguiendo el principio de "trimar
% NaN antes de transformar/estimar").

%% 1.1e) Gráfica exploratoria de las nuevas series EU
% ------------------------------------------------------------------------
fig_eu = figure('Name','Series EU: brecha IPI, inflacion CPI, FFR');
subplot(3,1,1); plot(dates_dt, brecha_ipi_eu, 'b','LineWidth',1.2); yline(0,'k:');
title(ipi_label, 'Interpreter','latex'); grid on;
subplot(3,1,2); plot(dates_dt, inf_eu, 'b','LineWidth',1.2); yline(0,'k:');
title('Inflaci\''on EU (log-diff CPI)', 'Interpreter','latex'); grid on;
subplot(3,1,3); plot(dates_dt, ffr, 'b','LineWidth',1.2);
title('Fed Funds Rate', 'Interpreter','latex'); grid on;
sgtitle('Variables EU candidatas a EXOG', 'Interpreter','latex');
print(fig_eu, '-dpdf', 'graphics/series_eu_exog.pdf');

%% 1.2) Chequeo de estacionariedad (ADF + KPSS) de todas las series
% ------------------------------------------------------------------------
series_test = {actividad, inf, tasa, exp, brecha_tcr, mps, brecha_ipi_eu, inf_eu, ffr};
label_test  = {'Brecha del producto (IGAE)', 'Inflaci\''on (sub)', 'Tasa de inter\''es', ...
               'Exp. Inflaci\''on', 'Brecha del TCR', 'mps (instrumento)', ...
               'Brecha del producto EU (IPI)', 'Inflaci\''on EU (CPI)', 'FFR'};
name_test   = {'actividad','sub','tasa','exp','brecha_tcr','mps','brecha_ipi_eu','inf_eu_pct','ffr'};
...
DUM_covid = zeros(T,1);
DUM_covid(dates_dt == datetime(2008,10,1) | dates_dt == datetime(2008,11,1) | dates_dt == datetime(2008,12,1) | dates_dt == datetime(2009,2,1) |dates_dt == datetime(2009,3,1) | dates_dt == datetime(2009,4,1) | dates_dt == datetime(2015,9,1) |  dates_dt == datetime(2015,10,1) |  dates_dt == datetime(2016,11,1) |  dates_dt == datetime(2017,1,1) | dates_dt == datetime(2017,2,1) | dates_dt == datetime(2017,3,1) | dates_dt == datetime(2020,4,1) | dates_dt == datetime(2020,5,1)) = 1;
...
%% 1.6) Bloque exógeno EU (reduced-form control, no participa en identificación)
% ------------------------------------------------------------------------
% brecha_ipi_eu, inf_eu_pct y ffr entran como controles exógenos junto con
% la dummy COVID. VARmodel solo acepta UNA matriz EXOG, así que se
% concatenan por ventana en la sección 6 (EXOGk = [DUMk, EXOG_us(idx{k},:)]).
EXOG_us     = [brecha_ipi_eu, inf_eu, ffr];
EXOGmnem_us = {'brecha_ipi_eu','inf_eu','ffr'};
nlag_ex     = 1;   % contemporáneo únicamente, igual que la dummy COVID
...
% ------------------------------------------------------------------------
n_ventanas = 1;
ventana_ini = datetime(2014,1,1);
ventana_fin = datetime(2026,6,30);   % <-- corregido: fin de año, no 1-ene
ventana_lbl = {'2008-2026'};                       % <-- corregido: coincide con fechas reales
...
% --- Descartar filas con NaN en el bloque exógeno (p.ej. la primera
%     observación de inf_eu_pct, perdida por el diff) de cualquier
%     ventana que la contenga. Mismo principio que el trim de NaN previo
%     al HP filter: nunca dejar NaN entrar a VARmodel.
fila_exog_valida = all(~isnan(EXOG_us), 2);
for k = 1:n_ventanas
    n_antes = length(idx{k});
    idx{k} = idx{k}(fila_exog_valida(idx{k}));
    if length(idx{k}) < n_antes
        fprintf('Ventana %d (%s): se descartaron %d observación(es) por NaN en el bloque EXOG (EU).\n', ...
            k, ventana_lbl{k}, n_antes - length(idx{k}));
    end
end
...
% n_exog_cols = 1 (dummy COVID) + 3 (brecha_ipi_eu, inf_eu, ffr)
n_exog_cols = 1 + size(EXOG_us,2);
n_param_por_ecuacion = nvars*nlags + detc + n_exog_cols*(nlag_ex+1);
for k = 1:n_ventanas
...
VARopt.mnem      = Xmnem;
VARopt.vnames_ex = {'Dummy COVID','Brecha IPI EU','Inflaci\''on EU','FFR'};
...
for k = 1:n_ventanas
    ...
    EXOGk = [DUMk, EXOG_us(idx{k}, :)]; % <-- dummy COVID + brecha_ipi_eu + inf_eu + ffr
    VARopt.IV      = mpsk;
    VARopt.figname = sprintf('graphics/svar_sign_iv_trimestral_%s', strrep(ventana_lbl{k}, '-', '_'));
    VAR_sub{k} = VARmodel(Xk, nlags, detc, VARopt, EXOGk, nlag_ex);
...
for k = 1:n_ventanas
    ...
    VAR_68{k} = VARmodel(Xk, nlags, detc, VARopt_68, EXOGk, nlag_ex);
    VAR_90{k} = VARmodel(Xk, nlags, detc, VARopt_90, EXOGk, nlag_ex);
end
